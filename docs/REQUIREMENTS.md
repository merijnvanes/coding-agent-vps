...
- **WON'T (v1)** — Package publishing (npm, PyPI, Docker Hub). The 0.1% of users who publish from their agent VPS can add the integration themselves following the pattern of `cloudflare-token` / `hcloud-token` in `daemon/cred-daemon.sh`. Docker Hub specifically also needs a multi-value secret (username + password) and a pre-exec login step — non-trivial.
