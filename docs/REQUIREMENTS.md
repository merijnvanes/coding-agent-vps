...
- **WON'T (v1)** — Package publishing (npm, PyPI, Docker Hub). The 0.1% of users who publish from their agent VPS can add the integration themselves: store the token in the `coding-agent-vps-tooling` Infisical project, add a PATH shim at `sandbox/wrappers/<cli>` that does `infisical run -- <real-binary> "$@"`. Docker Hub specifically also needs a multi-value secret (username + password) and a pre-exec login step — non-trivial.
...
- **MUST** — Raw host-side credential files at rest live in the creds zone (`/var/lib/agent-vps/creds/`, mode 0600, owned by the `creds` user) — the sandbox cannot read them directly. Scope: `github-ssh-key` (loaded into ssh-agent) and `ntfy-topic` (read by the alert script). No cloud-CLI tokens live here.
...
- **MUST** — Sandbox-side cloud-CLI tokens (stored in Infisical under the env-var names the CLIs read: `HCLOUD_TOKEN`, `CLOUDFLARE_API_TOKEN`, `GCP_SA_KEY_JSON`, `SUPABASE_ACCESS_TOKEN`, future similar) are NOT persisted on the VPS at rest. PATH shims at `/opt/agent-vps-wrappers/` fetch them via `infisical run --` per command-invocation; tokens enter the subprocess environment for the duration of one command, then gone.
- **Acknowledged** — For one command's lifetime, the matching token is in the subprocess's env (readable via `/proc/<pid>/environ` from a sibling process). The `gcloud` shim additionally writes the SA JSON to a `/dev/shm/<random>` temp file (mode 0600, tmpfs only, removed on EXIT/HUP/INT/TERM trap). Only SSH is a true signing oracle.
- **MUST** — The Infisical Universal Auth bootstrap secret (for the `coding-agent-vps` project) is stored in a path readable only by the cred-daemon's user (`/etc/agent-vps/infisical-uauth`, mode 0600, owned by the `creds` user). The sandbox container has no mount or read access to this path. The sandbox-side Infisical authentication is interactive (`infisical login` device-code OAuth) and uses the user's personal Infisical identity — no machine credential lives in the sandbox at rest.
...
- **MUST** — The host-side VPS's access to Infisical uses a credential with bounded TTL (Infisical Universal Auth access tokens), not a permanent token.
- **MUST** — Host-side credential values (`github-ssh-key`, `ntfy-topic`) are stored in the `coding-agent-vps` Infisical project and fetched by the cred-daemon. The cred-daemon does not mint, rotate, or otherwise manage upstream credential lifecycles.
- **MUST** — Sandbox-side cloud-CLI tokens are stored in the `coding-agent-vps-tooling` Infisical project and fetched per-command-invocation by PATH shims via `infisical run --`. App-specific secrets live in per-app Infisical projects, also fetched per-command.
- **SHOULD** — The cred-daemon refreshes its host-side local cache from Infisical daily so user-initiated changes to `github-ssh-key` / `ntfy-topic` propagate within 24h. Sandbox-side rotation is immediate (every shim invocation re-fetches).
...
- **MUST** — I can revoke the VPS's access to Infisical from outside the VPS via Infisical's admin interface, for each of the three project-grants independently: the cred-daemon's Universal Auth identity (host-side, `coding-agent-vps`), and my personal Infisical access (sandbox-side, `coding-agent-vps-tooling` and per-app projects).
- **MUST (v1)** — Revoking the cred-daemon's Universal Auth identity causes its next refresh to fail. Revoking sandbox-side Infisical access (or rotating personal-account auth) causes the next `infisical run --` / shim invocation to fail. Plus immediate "kill the VPS from Hetzner panel" as a hard backstop — the Hetzner panel uses a separate admin credential kept on the laptop (per §4), not any sandbox or VPS-held token, so the agent cannot prevent this backstop even if fully compromised.
- **SHOULD** — A documented "scrub local cache" procedure exists for incidents: stop the cred-daemon, remove `/var/lib/agent-vps/creds/*`, then `docker compose up -d --build` (or `docker rm sandbox`) to recreate the sandbox container — which discards `~/.infisical/` along with everything else in the container's writable layer. `docker stop` alone does NOT discard it (the writable layer persists until container removal/recreation). Revoking in Infisical alone does NOT invalidate values already in a running subprocess's env at the moment of revocation, but the subprocess lifetime is bounded to one command — no shell-resident long-lived copies.
