...
- **Resource limits** (adaptive to host RAM): `scripts/cloud-init-tasks.sh` step 1 detects host RAM at provision time and writes `/opt/agent-vps/.env` with `SANDBOX_MEM_LIMIT=<MemTotal − 1024 MB>m`; `docker-compose.yml` consumes that via `mem_limit: ${SANDBOX_MEM_LIMIT:-3g}` (3 GB default approximately matches a cx23 ~4 GB host minus firmware reserve). On cx33 (~8 GB host) the container gets ~6.9 GB; on cx43 (~16 GB) it gets ~15 GB. The 1 GB host reserve is fixed because sshd/dockerd-rootless/tailscaled don't grow with available RAM. `cgroup_parent: sandbox.slice` places the container under a systemd user slice (`daemon/sandbox.slice`) that enforces `MemorySwapMax=0` (denies the container any swap allowance, keeping the host's 2 GB swap reserved as its own emergency reclaim) plus `MemoryHigh=<mem_limit × 0.9>M` applied at install time via `systemctl --user set-property` (~90% soft-throttle threshold). The slice approach is required because Docker's `memswap_limit` field is silently ignored on rootless Docker + cgroup v2; the slice gives a working enforcement path. When breached, in-container allocations get `ENOMEM` at the cgroup boundary — most well-behaved runtimes (Python, Node.js) surface this as a language-level out-of-memory error and exit; the cgroup OOM killer reaps anything that doesn't self-terminate as a backstop. The host stays responsive throughout. Paired with a 2 GB host swapfile + `vm.swappiness=10` (constant regardless of host size) which gives the host's own reclaim somewhere to push cold pages instead of evicting file cache — the actual mechanism behind the page-cache-thrashing lockup mode (see [USAGE.md "VPS unresponsive"](./USAGE.md#vps-unresponsive-cpu-pegged-ssh-hangs)).
