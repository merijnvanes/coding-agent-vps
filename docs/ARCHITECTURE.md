...
- **Resource limits**: `mem_limit: 3g` in `docker-compose.yml` bounds the container's cgroup to 3 GB of the host's 3.7 GB RAM. `cgroup_parent: sandbox.slice` places the container under a systemd user slice (`daemon/sandbox.slice`) that enforces `MemorySwapMax=0` — denying any swap allowance, so the host's 2 GB swap stays reserved for the host's own emergency reclaim. The slice approach is required because Docker's `memswap_limit` field is silently ignored on rootless Docker + cgroup v2 (it sets `HostConfig.MemorySwap` but never applies it to the cgroup); the slice gives a working enforcement path. When breached, in-container allocations get `ENOMEM` at the cgroup boundary — most well-behaved runtimes (Python, Node.js) surface this as a language-level out-of-memory error and exit; the cgroup OOM killer reaps anything that doesn't self-terminate as a backstop. The host stays responsive throughout. Paired with a 2 GB host swapfile + `vm.swappiness=10` (set by `scripts/cloud-init-tasks.sh` step 1) which gives the host's own reclaim somewhere to push cold pages instead of evicting file cache — the actual mechanism behind the page-cache-thrashing lockup mode (see [USAGE.md "VPS unresponsive"](./USAGE.md#vps-unresponsive-cpu-pegged-ssh-hangs)).
